...
```python
sns.factorplot(x="PersonType", hue="Survived", data=train, kind="count", size=6)
...
```python
sns.factorplot(x="Pclass", hue="Survived", data=train, kind="count", size=6, palette="Set2")
...
```python
sns.factorplot(x="Pclass", y="Survived", hue="PersonType", data=train, kind="bar", aspect=1.5, size=6, palette="Set2")
...
```python
sns.factorplot(x='Title', hue='Survived', data=train, kind='count', aspect=1.5, size=6)
...
```python
sns.factorplot(x='TotalFamily', y='Survived', data=train, kind='bar', aspect=1.5, size=6, palette="Greens")
...
That wraps up EDA. In my next post, I will take this data through a machine learning workflow. The focus of the post will be on _process_ - the journey and not the destination 8) - so I would look at additional resources if you are trying to make a dent on the Kaggle leaderboard. Stay tuned!
